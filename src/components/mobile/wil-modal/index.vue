...
const showModal = obj => {
  ...
  data.value = { ...defaults, ...obj }
  // 取消按钮颜色：优先使用调用方传入的值，否则跟随主题
  if (!data.value.cancelColor) {
    data.value.cancelColor = iconColor.value
  }
  show.value = true
}
...
.wil-modal {
  ...
  .wil-modal-button {
    ...
    .wil-modal-button__cancel {
      ...
      height: 120rpx;
      border-right: 2rpx solid var(--app-border-strong);
