    ...
    <div :class="['add-emby-form', themeClass]">
        ...
        <div class="add-emby-form__container">
            ...
            <nut-popup v-model:visible="showProtocol" position="bottom" safe-area-inset-bottom round>
                <nut-picker v-model="protoValue" :columns="protoColumns" :title="t('source.selectProtocol')" @confirm="confirmPicker"
...
const title = ref("");
const isAdd = computed(() => title.value == '添加Emby' || title.value == t('navbar.addEmby'))
const titleText = computed(() => isAdd.value ? t('navbar.addEmby') : t('navbar.modifyEmby'))
...
const confirmSubmit = () => {
    base_form.value.confirmCommit().then(async (valid) => {
        if (valid) {
            validateEmby(isAdd.value, state.formData, state.oldData, routerParams.value) //校验，抽成一个方法了
        }
    });
};
