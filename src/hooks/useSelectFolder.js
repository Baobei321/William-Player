...
export function useSelectFolder({ selectType, selectMedia, result, title, emits }) {
    ...
    const getFileList = async (data) => {
        if (selectType.type == "WebDAV") {
            ...
            return new Promise((resolve) => {
                uni.request({
                    url: selectMedia.protocol + "://" + selectMedia.address + ":" + selectMedia.port + "/api/fs/list",
                    data: JSON.stringify({ path: path.value ? path.value : '/', page: data.pageNum, per_page: data.pageSize, refresh: false }),
                    method: "POST",
                    header: { Authorization: selectMedia.token, "Content-Type": "application/json" },
                    success: (res) => {
                        resolve(res.data);
                    },
                });
            ...
            let res = await get189Folder({ ...data, folderId: folderFileId.value }, selectMedia);
            ...
            let res = await getQuarkFolder({ ...data, fid: folderFileId.value }, selectMedia);
    ...
    const confirm = () => {
        ...
        if (selectType.type == 'WebDAV') {
            let exit = muluData.value[mapping[title]].find(v => v.type == selectType.type && v.name == selectMedia.name)
            //已存在就替换，不存在就新增
            exit ? exit.path = path.value + '/' + selectName.value || '/' : muluData.value[mapping[title]].push({ type: selectType.type, name: selectMedia.name, path: path.value + '/' + selectName.value || '/' })
        } else {
            let exit = muluData.value[mapping[title]].find(v => v.type == selectType.type && v.name == selectMedia.name)
            //已存在就替换，不存在就新增
            if (exit) {
                ...
                let obj = { type: selectType.type, name: selectMedia.name, path: path.value + '/' + selectName.value || '/', folderFileId: folderFileId.value }
                muluData.value[mapping[title]].push(obj)
            }
        }
    ...
    return { data, key, selectName, responseAdapter, handleData, handleSize, chooseName, toBack, getFileList, clickCell, setImg, confirm }
}