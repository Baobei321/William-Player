...
(function() {
	...
	QRCode = function(opt) {
		...
		options = {
			...
			padding: Math.floor(opt.padding) || 10,
			fileType: opt.fileType || "png"
		};
		...
		if (opt) {
			for (var i in opt) {
				...
				// 处理二维码大小
				if (i == "size") {
					options[i] = opt.size - options.padding * 2
				}
			}
		}
		//使用QRCodeAlg创建二维码结构
		...
		let createCanvas = function(options) {
			...
			if (options.image) {
				...
				drawRoundedRect(ctx, x + options.padding, y + options.padding, ratioImgSize, ratioImgSize, 2, 6, true, true)
				ctx.drawImage(options.image, x + options.padding, y + options.padding, ratioImgSize, ratioImgSize);
				// 画圆角矩形
