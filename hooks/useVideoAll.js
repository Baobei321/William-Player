...
export function useVideoAll({ wil_list, wil_modal }) {
    ...
    const getEmbyJjList = async (params) => {
        ...
        let arr = res.Items.map(i => {
            return {
                ...
                poster: i.ImageTags.Primary ? `${selectMedia.value.protocol}://${selectMedia.value.address}:${selectMedia.value.port}/emby/Items/${i.Id}/Images/Primary?tag=${i.ImageTags.Primary}` : posterEmpty
            }
        })
        return {
            code: "200",
    ...
    const getEmbyGenreList = async (params) => {
        ...
        let arr = res.Items.map(i => {
            return {
                id: i.Id, name: i.Name, provider: 'Emby', notShowTime: true, isGenre: true,
                poster: i.ImageTags.Primary ? `${selectMedia.value.protocol}://${selectMedia.value.address}:${selectMedia.value.port}/emby/Items/${i.Id}/Images/Primary?tag=${i.ImageTags.Primary}` : posterEmpty
            }
        })
        return {
    ...
    const getEmbyFolderList = async (params) => {
        ...
        let arr = res.Items.map(i => {
            return {
                id: i.Id, name: i.Name, provider: 'Emby', notShowTime: true, isFolder: true,
                poster: i.ImageTags.Primary ? `${selectMedia.value.protocol}://${selectMedia.value.address}:${selectMedia.value.port}/emby/Items/${i.Id}/Images/Primary?tag=${i.ImageTags.Primary}` : posterEmpty
            }
        })
        return {
    ...
    return {
        routerParams, showPopover, editHistory, isSelect, mapping, changeSort, isClearAll, tabList, changeTab,
