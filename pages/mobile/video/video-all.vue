  ...
  if (isSelect.value) {
    ...
    if (routerParams.value.title != "最近观看") {
      ...
      if (item.type == "movie") {
        ...
        if (selectType.value.type == "WebDAV") {
          uni.navigateTo({
            url: `/pages/mobile/video/video-player?path=${item.path}&titlePlay=${item.titlePlay}&type=tv&${toStringfy(openEndTime)}`,
            success: () => {
              setItemFirst(item);
            },
          });
          ...
          uni.navigateTo({
            url: `/pages/mobile/video/video-player?path=${item.path}&folderFileId=${item.folderFileId}&titlePlay=${item.titlePlay}&type=tv&${toStringfy(openEndTime)}`,
          });
        }
      }
    }
