		...
		<div class="video-detail-container">
			...
			<div class="video-detail-container__content">
				...
				<div class="movie-version" v-if="routerParams.type=='movie'">
					...
					<scroll-view class="movie-version-scroll" :scroll-x="true" style="width: 100%" :enhanced="true"
						...
						<div class="movie-version-list">
							<div :class="['movie-version-list__item',item.provider==selectSource.provider ? 'movie-version-list__active' : '']"
								v-for="item in sourceList" :key="item.provider" @click="changeSource(item)">
								{{ item.sourceName }}
							</div>
						</div>
					</scroll-view>
				...
				<div class="tv-version" v-if="routerParams.type=='tv'">
					<nut-tabs v-model="activeTab" :title-scroll="true" custom-color="#090909" background="#fff"
						@change="changeTvSource">
						<nut-tab-pane :title="item.sourceName" :pane-key="item.provider" v-for="item in sourceList"
							:key="item.provider"></nut-tab-pane>
					</nut-tabs>
					<scroll-view class="tv-version-scroll" :scroll-x="true" style="width: 100%" :enhanced="true"
						:showScrollbar="false">
						<div class="tv-version-list">
							<div class="tv-version-list__item" v-for="(item,index) in tvList" :key="item.name"
				...
				<div class="tip-footer">
					...
					<div class="tip-footer-timesize">
						<span
							v-if="imgData.runtime">{{ imgData.runtime }}</span><span>{{ selectSource.sourceName }}</span><span
...
<script setup>
	...
	const handleTv = async () => {
		...
		if (selectType.value.type == "WebDAV") {
			result = await getFolder({
				path: selectSource.value.path + "/" + selectSource.value.name
			}, selectMedia.value);
			//对电视进行排序
		...
		tvList.value.forEach((v, vindex) => {
			...
			v.poster = res1.episodes[vindex]?.still_path ?
				"https://media.themoviedb.org/t/p/w533_and_h300_bestv2" + res1.episodes[vindex]
				?.still_path : "";
			v.runtime = res1.episodes[vindex]?.runtime ? calTime(res1.episodes[vindex]?.runtime, "en") :
				"00:00";
		});
	};
	...
	const clickPlayButton = () => {
		if (routerParams.value.type == "movie") {
			let history = historyPlay.value?.find((i) => getMovieName(i.name) == getMovieName(selectSource.value
			.name));
			if (history) {
				if (selectType.value.type == "WebDAV") {
					uni.navigateTo({
			...
			if (history) {
				if (selectType.value.type == "WebDAV") {
					...
					uni.navigateTo({
						url: `/pages/video/video-player?path=${selectSource.value.path.slice(1)}/${selectSource.value.name}/${history.name}&folderFileId=${history.folderFileId}&type=tv`,
					});
				}
			} else {
				let historyItem = {
