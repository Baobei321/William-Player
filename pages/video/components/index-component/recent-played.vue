...
const toVideoPlayer = async (item) => {
  ...
  if (item.type == "movie") {
    ...
    if (selectType.value.type == "WebDAV") {
      ...
      uni.navigateTo({
        url: `/pages/video/video-player?path=${item.path}&titlePlay=${item.titlePlay}&wjjId=${item.wjjId}&movieTvId=${item.movieTvId}&folderFileId=${item.folderFileId}&type=tv&${toStringfy(openEndTime)}`,
      });
    }
  }
};
